            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="filter-cont mb-3">
                        <form action="{{ url('hr/candidate/list/filter')}}" method="get">
                            <div class="row">
                                <div class="col-md-12">
                                 ...
                                 <div class="col-md-3">
                                    <label for="inputName">Hr Name</label>
                                        <select name="hr_name" class="form-control">
                                            <option value="">Select Name...</option>
                                            @foreach($hrs as $key=>$val)
                                            <option value="{{$val->id}}" {{ (isset($hr_name) && $hr_name == $val->id) ? 'selected' : '' }}>{{$val->name}}</option>
                                            @endforeach
                                        </select>
                                </div>
                                <div class="col-md-3">                                
                                ...
                                <table class="table table-bordered" width="100%" cellspacing="0" id="myTable">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($details->status == 0)
                                                <!-- <a class="btn btn-rounded btn-outline-dark mr-3 ml-auto" href="#" onclick="changeStatus(<?=$details->id?>, 1)">
                                                </a> -->
                                                <div  class="btn btn-rounded btn-outline-dark mr-3 ml-auto" data-toggle="modal" data-target="#exampleModalStatus{{ $details->id }}" data-statusid="{{ $details->id }}">Pending</div>
                                                @endif
                                                @if($details->status == 1)
                                                <!-- <a class="btn btn-rounded btn-success mr-3 ml-auto" href="#" onclick="changeStatus(<?=$details->id?>, 2)">
                                                Selected</a> -->
                                                <div  class="btn btn-rounded btn-success mr-3 ml-auto" data-toggle="modal" data-target="#exampleModalStatus{{ $details->id }}" data-statusid="{{ $details->id }}">Selected</div>
                                                @endif
                                                 @if($details->status == 4)
                                                <!-- <a class="btn btn-rounded btn-success mr-3 ml-auto" href="#" onclick="changeStatus(<?=$details->id?>, 2)">
                                                Selected</a> -->
                                                <div  class="btn btn-rounded btn-success mr-3 ml-auto" data-toggle="modal" data-target="#exampleModalStatus{{ $details->id }}" data-statusid="{{ $details->id }}">Joined</div>
                                                @endif
                                                @if($details->status == 2)
                                                <!-- <a class="btn btn-rounded btn-outline-danger mr-3 ml-auto" href="#" onclick="changeStatus(<?=$details->id?>, 0)">Rejected</a> -->
                                                <div  class="btn btn-rounded btn-outline-danger mr-3 ml-auto" data-toggle="modal" data-target="#exampleModalStatus{{ $details->id }}" data-statusid="{{ $details->id }}">Rejected</div>
                                                @endif
                                                @if($details->status == 3)
                                                <!-- <a class="btn btn-rounded btn-outline-danger mr-3 ml-auto" href="#" onclick="changeStatus(<?=$details->id?>, 0)">Rejected</a> -->
                                                <div  class="btn btn-rounded mr-3 ml-auto" data-toggle="modal" data-target="#exampleModalStatus{{ $details->id }}" data-statusid="{{ $details->id }}" style="background-color:gray; border-color: gray;color: white;">Not Joined</div>
                                                @endif
                                                
                                            </td>
                                            <td>
                                                @if($details->status == 0)
                                                <div  class="btn btn-primary" data-toggle="modal" data-target="#exampleModal{{ $details->id }}" data-rid="{{ $details->id }}">Reschedule Interview</div>
                                                @endif
                                            </td>
                                            <td>
                                        ...
                                        <div class="modal fade exampleModalStatus" id="exampleModalStatus{{ $details->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                            <div class="modal-dialog" role="document">
                                                <form action="{{ url('/hr/candidate/interview/status') }}" method="post">
                                                    @csrf
                                                                    ...
                                                                    <div class="form-group">
                                                                        ...
                                                                        <select name="status" class="form-control">
                                                                            ...
                                                                            <option value="1" {{ ($details->status == 1) ? 'selected' : '' }}>Selected</option>
                                                                             <option value="4" {{ ($details->status == 4) ? 'selected' : '' }}>Joined</option>
                                                                            <option value="2" {{ ($details->status == 2) ? 'selected' : '' }}>Rejected</option>
                                                                            <option value="3" {{ ($details->status == 3) ? 'selected' : '' }}>Not Joined</option>
                                                                        </select>
                                                                    </div>
                                                                    <div class="form-group" id="datef">
                                                                        <label for="joingDate"></label>
                                                                        <input type="date" name="joining_date" class="form-control" value="{{$details->joining_date}}">
                                                                    </div>
                                                                </div>
                                                                <div class="col-sm-6">
                                                                    <div class="form-group">
                                        ...
                                        <div class="modal fade exampleModal" id="exampleModal{{ $details->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                            <div class="modal-dialog" role="document">
                                                <form action="{{ url('/hr/candidate/assign-interview/update') }}/{{$details->id}}" method="post">
                                                    @csrf
                                                    <input type="hidden" name="modal" value="1">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
...
<script>
    function changeStatus(id, status) {
        $.ajax({
            url: "{{ url('/hr/candidate/interview/status') }}",
            type:'POST',
            data:{
                id:id, 
                status:status, 
